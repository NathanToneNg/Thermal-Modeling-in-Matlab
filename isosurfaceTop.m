%isosurfaceTop: Plots the matrix provided (or finalTemps if none is given)
%   in an isosurface plot, plotting the 91st through 100th percentiles
%
...
for i = 1:10
    ...
    p = patch(isosurface(X,Y,Z,V,prctile(reshape(V,numel(X),1,1), 90 + i))); % isosurfaces at top 10% (91-100)
...
textboxString2 = sprintf('91st Prctile: %9.3f', prctile(reshape(V,numel(X),1,1), 91));
annotation('textbox',[0.05,0.85,0.1,0.1], 'String',{textboxString1, textboxString2});
view(3)
hold off;

%Warmest is maximum. Each shade green-er is a tenth
%of the range less, down to 91st percentile. 
